/**
 * @brief: 	modem.c - Modem utility functions for nRF LTE modem
 *
...
#include <modem/pdn.h>      // PDN library for defining/modifying PDP context information (ie: APN)
#include <zephyr/sys/reboot.h>     // Library functions to reboot system
#include <zephyr/logging/log.h>    

// Citysage specific includes

#include "sys_wrapper.h"
#include "modem.h"
#include "led.h"

// register this module with the logging package
...
/*
* The size of buffer used to store responses from AT commands sent to modem
*/
#define MODEM_RESPONSE_STRING_SZ    64
...
/*
*   Value to indicate now RSRP available
*/
#define     RSRP_NO_VALUE   -255
...
/** 
* @brief    modem_pdp_control - Modem function to bring the PDP conext up or down - currently used for debugging Cloud connectivity issues 
...
void modem_get_imei(char *device_imei, int imei_size)
{
    ...
    if (ret < 0)
        erabort("modem_get_imei");
}

/**
 * @brief modem_get_iccid - Acquires the ICCID of the SIM card
 * 
 * @param iccidp    pointer to string buffer to store ICCID, gets updated if it is a success
 * @param iccid_size size of string buffer to store ICCID 
 * 
 * @return          nothing
 * 
 * @note:           Aborts if unable to access modem 
 */
void modem_get_iccid(char *iccidp, int iccid_size)
{
    int ret;
    char response[MODEM_RESPONSE_STRING_SZ];    // buffer to store modem responses from AT commands 

    //CFUN=41 is to activate UICC (ie: the SIM card) which is needed to obtain ICCID from the UICC
    if (nrf_modem_at_cmd(response, sizeof(response), "AT+CFUN=41"))
        erabort("modem.c: Failed activate UICC");
        
    // Now we can get the ICCID of the SIM card
    ret = modem_info_string_get(MODEM_INFO_ICCID, iccidp, iccid_size);

    if (ret < 0)
    {
        // NOTE: If we failed to get the ICCID, we can't abort because perhaps the SIM is not installed
        LOG_ERR("failed to get ICCID -> err:%d", ret);

        // Given no ICCID is available, let's assume there's no UICC installed (likely) and return to caller's buffer
        strncpy(iccidp, "No UICC installed", iccid_size); 
    }

    // don't forget to turn off the UICC/SIM card
    nrf_modem_at_cmd(response, sizeof(response), "AT+CFUN=40");
} 

/**
 * @brief modem_get_operator - Returns the name or MCC/MNC of the current network operator (if attached)
 * 
 * @param namep    pointer to string buffer to store name, gets updated if it is a success
 * @param name_sz  size of string buffer to store name
 * 
 * @return          nothing
 * 
 * @note:            
...
void modem_get_operator(char *namep, int name_size)
{
    ...
    err = modem_info_string_get(MODEM_INFO_OPERATOR, namep, name_size);
    
    // if we can't fetch the name (ex: scanning), just say unavailable
    if (err < 0)
        strncpy(namep, "Not available", name_size);
}
/**
 * @brief   modem_enable_rsrp_monitor - Enables the monitoring of RSRP values (non-blocking function)
 * 
 * @param   void 
 * 
 * @return  nothing
...
void modem_enable_rsrp_monitor(void)
{
    ...
    if(modem_info_rsrp_register(modem_rsrp_cb))
        erabort("modem.c: modem_enable_rsrp_monitor");
}

/**
 * @brief   modem_get_rsrp_dbm_now - Get the RSRP value (blocking call)
 * 
 * @param   void
 * 
...
int modem_get_rsrp_dbm_now(void)
{
    ...
    // get the RSRP value, negative return value is error
    len = modem_info_short_get(MODEM_INFO_RSRP, &rsrp);

    if (len < 0) 
        // not available
        ret = RSRP_NO_VALUE;
    else
        // convert raw value to dbm for the caller
        ret = modem_rsrp_raw_to_dbm(rsrp);
    return ret;
}

/**
 * @brief modem_get_rsrp_dbm - Returns the most updated RSRP value
 * 
 * @return Returns RSRP value, 0 is the initial value (if called before RSRP callback runs)
 * 
 * @note: modem_enable_rsrp_monitor must be called to start updates
 * 
 * TODO: we could add a check that the callback has been run once (using an flag but 0 might suffic for now)
 */
...
/**
 * @brief   modem_set_state - Configures the power state of the Modem
 * 
 * @param   state Desired state of the Modem
 * 
 * @return  error code: 0 is success
 * 
 * @note:   1 - Unsure if this is used, deprecate?
 *          2 - If the modem sub-system does give us an error, we don't abort given this might be 
 *              used by the factory test code and perhaps it possible.
 *          3 - Aborts if caller provides a bogus desired state.   
 */
int modem_set_state(modem_state_t state)
{
    ...
    switch(state)
    {
        ...
        case MODEM_OFFLINE:
            err = lte_lc_offline(); // less flash write than turning it off completely
            break;

        default:
            erabort("modem.c: modem_set_state");
            err = -1;
            break;
    }
...
*
*           Aborts if unable to correctly initialize the modem
*     
*/
void modem_init_apn_pdp_context(void (*pdp_change_callbackp) (bool flag))
{
    int err;
	...
	if (err) 
        erabort("modem.c: failed at+cnec cmd");

	err = nrf_modem_at_printf("AT+CGEREP=1");
	if (err) 
        erabort("modem.c:failed at_cgerep cmd");

	// initialize the PDN library
	err = pdn_init();
	if (err) 
        erabort("modem.c: failed pdn_init");
	...
	err = pdn_default_callback_set(pdn_event_handler);
        if (err)
            erabort("modem.c: failed pdn_callback reg");
    ...
    */
	...
	if (err) 
        erabort("modem.c: failed pdn configure");
...
/** 
* @brief    modem_lookup_apn - Helper function to determine the APN and PDN type based on the IMSI of the SIM card.
*          
* @param    void
*
* @return   none
*
* @note     The requirement is to automatically set the PDN on the modem based on the SIM card being used. Previously this was a compile
*           time setting. We want to be able to change SIM cards without making any code changes or configuration changes to the device. 
*/
static void modem_lookup_apn(void)
{
    ...
    // get the IMSI from the SIM card in order to determine what Operator the SIM card belongs to
	modem_info_string_get(MODEM_INFO_IMSI, imsi_str, LTE_IMSI_LEN);
	LOG_DBG("SIM IMSI: %s", imsi_str);

    match = false; // setup loop termination flag

    // Loop thru look-up table until we find a MNC/MCC match OR until we get to the end of the table
    for (i=0; (i < lookup_tbl_sz) && !match; i++)
    {
        ...
        if (0 == memcmp(imsi_str, pdn->mccmnc_characters, MCCMNC_LEN))
            match = true;       // ok, we found a match in the table, we are done
    }
...
/** 
* @brief    pdn_event_handler - Event callback handler for Nordic's PDN module 
*          
...
static void pdn_event_handler(uint8_t cid, enum pdn_event event, int reason)
{
	...
	default:
		// Perhaps we should abort here as we should never get here unless the modem is reporting undefined values. It might be possible
        // over time with new SDKs that new codes could be added by Nordic so let's tolerate that situation and just log it. 
        LOG_WRN("PDP Event undefined. Event: %d", event);
		break;
	}
}

/**
 * @brief modem_rsrp_raw_to_dbm - Converts raw RSRP to dBm
 * 
...
/**
 * @brief modem_rsrp_cb - RSRP monitoring callback function. Called asynchronously from the nRF modem handling code
 * 
 * @param current_rsrp new/latest value of RSRP
 * 
 * @return Does not return any
 *
 * @note    Caches the value to allow for non-blocking access to the latest RSRP value to our application
 */
static void modem_rsrp_cb(char current_rsrp)
{
    //convert RSRP to dBm
